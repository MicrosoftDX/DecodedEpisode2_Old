...
function renderContributors(contributors) {
    ...
    for (var index = 0; index < contributors.length; index++) {
        ...
        tableRow.innerHTML = "<td><img src='" + contributor.avatar_url + "' /></td>" + "<td>" + contributor.login + "</td>" + "<td>" + contributor.contributions + "</td>";
...
function renderRepos(repos) {
    ...
    for (var index = 0; index < repos.length; index++) {
        ...
        var tableRow = document.createElement("tr");
        tableRow.innerHTML = '<td><a data-repo="' + repo.name + '" href="#" class="favorite btn ' + btnClass + '"><span class="glyphicon glyphicon-star"></span></a></td>' + "<td><a class='repo' href='#'>" + repo.name + "</a></td>";
        container.appendChild(tableRow);
    }
...
document.addEventListener("DOMContentLoaded", function () {
    Ajax.request("/identitycreds", "GET", null, null, function (result) {
